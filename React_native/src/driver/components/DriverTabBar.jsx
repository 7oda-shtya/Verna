...
// Mirrors src/client/components/Navigation.jsx's floating pill tab bar exactly (same styling,
// icon library, active/inactive theme tokens, spring animation) with the driver's own tab set and
// its own "active trip" signal (state.trip.activeTrip, not the client's currentTrip.status).
const TABS = [
  { name: 'Home', icon: 'home-outline', activeIcon: 'home', label: 'الرئيسية' },
  { name: 'MyOffers', icon: 'pricetags-outline', activeIcon: 'pricetags', label: 'عروضي' },
...
const TabButton = ({
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      onLayout={onLayout}
      style={{ minHeight: 54, opacity: disabled && !isFocused ? 0.55 : 1, flex: 1, alignItems: 'center', justifyContent: 'center', gap: 2 }}>
      ...
      <Animated.Text style={[{ ...theme.typography.tiny, fontSize: 12 }, labelStyle]}>
        {tab.label}
      </Animated.Text>
    </Pressable>
  )
...
const DriverTabBar = ({ state, navigation }) => {
  const { theme } = useTheme()
  const { colors, shadows: elevation } = theme
  const insets = useSafeAreaInsets()
  const hasActiveTrip = useSelector(s => Boolean(s.trip?.activeTrip?.id))
