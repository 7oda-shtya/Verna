...
const ACCOUNT_ROLES = ['CLIENT', 'DRIVER'];
...
function validateRole(role) {
  if (!ACCOUNT_ROLES.includes(role)) throw new ApiError(400, 'نوع الحساب غير صالح');
}

async function validOtp(phone, role, code, purpose) {
  return prisma.otpCode.findFirst({
    where: { phone, role, code, purpose, verifiedAt: null, expiresAt: { gt: new Date() } },
    orderBy: { createdAt: 'desc' },
  });
}

export const requestOtp = catchAsync(async (req, res) => {
  const { phone, role, purpose = 'PHONE_VERIFICATION' } = req.body;
  if (!phone) throw new ApiError(400, 'رقم الهاتف مطلوب');
  validatePurpose(purpose);
  validateRole(role);
  const user = await prisma.user.findUnique({ where: { phone_role: { phone, role } }, select: { id: true } });
  if (!user) throw new ApiError(404, 'الحساب غير موجود');

  await prisma.otpCode.updateMany({
    where: { phone, role, purpose, verifiedAt: null },
    data: { expiresAt: new Date() },
  });
  const code = String(Math.floor(100000 + Math.random() * 900000));
  await prisma.otpCode.create({
    data: { phone, role, purpose, code, expiresAt: new Date(Date.now() + OTP_TTL_MS) },
  });
...
export const verifyOtp = catchAsync(async (req, res) => {
  const { phone, role, code, purpose = 'PHONE_VERIFICATION' } = req.body;
  validatePurpose(purpose);
  validateRole(role);
  const otp = await validOtp(phone, role, code, purpose);
  ...
  await prisma.$transaction(async tx => {
    ...
    if (purpose === 'PHONE_VERIFICATION') {
      await tx.user.update({ where: { phone_role: { phone, role } }, data: { isPhoneVerified: true } });
    }
  });
...
export const resetPassword = catchAsync(async (req, res) => {
  const { phone, role, code, newPassword } = req.body;
  ...
  const otp = await prisma.otpCode.findFirst({
    where: { phone, role, code, purpose: 'PASSWORD_RESET', expiresAt: { gt: new Date() } },
    orderBy: { createdAt: 'desc' },
  });
  if (!otp) throw new ApiError(400, 'رمز التحقق غير صحيح أو منتهي');
  const password = await bcrypt.hash(newPassword, 10);
  await prisma.$transaction([
    prisma.user.update({ where: { phone_role: { phone, role } }, data: { password } }),
